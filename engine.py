...
def train_one_epoch(args, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    itr_counter = 0
    for samples, depths, targets in metric_logger.log_every(data_loader, print_freq, header):
        itr_counter += 1
        total_itr_num += 1

        samples, depths = samples.to(device), depths.to(device)
        captions = [t["caption"] for t in targets]

        targets = utils.targets_to(targets, device)
        with amp.autocast(enabled=args.amp):
            outputs = model(samples, depths, captions, targets)
...
def evaluate_a2d(model, data_loader, postprocessor, device, args):
    ...
    for samples, depths, targets in metric_logger.log_every(data_loader, 100, header):
