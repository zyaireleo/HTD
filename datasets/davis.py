...
class DAVIS17Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            imgs, labels, boxes, masks, depths, valid = [], [], [], [], [], []
            for j in range(self.num_frames):
                frame_indx = sample_indx[j]
                frame_name = frames[frame_indx]
                img_path = os.path.join(str(self.img_folder), 'JPEGImages', video, frame_name + '.jpg')
                depth_path = os.path.join(str(self.img_folder), 'JPEGImages', video, frame_name + '.jpg')
            ...
            imgs,ds, target = self._transforms(imgs,depths, target)
            imgs = torch.stack(imgs, dim=0) # [T, 3, H, W]
            ds = torch.stack(ds, dim=0) # [T, 3, H, W]
