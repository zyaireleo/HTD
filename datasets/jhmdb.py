...
class JHMDBSentencesDataset(Dataset):
    ...
    def __init__(self, image_folder: Path, ann_file: Path, transforms, return_masks: bool,
        ...
        self.dataset_path = image_folder
    ...
    def __getitem__(self, idx):
        ...
        image_folder = os.path.join(self.dataset_path, 'Rename_Images')
        ...
        # read frames
        imgs, depths, boxes, masks, valid, labels = [], [], [], [], [], []
        for i in sample_indx:
            # p = '/'.join(chosen_frame_path.split('/')[:-1]) + f'/{i:05d}.png'
            p = '/'.join(chosen_frame_path.split('/')[-3:-1]) + f'/{i:05d}.png'
            frame_path = os.path.join(image_folder, p)
            depth_path = os.path.join(depth_folder, p)
            imgs.append(Image.open(frame_path).convert('RGB'))
            depths.append(Image.open(depth_path).convert('RGB'))

        # read the instance masks:
        # video_masks_path = os.path.join(self.dataset_path, video_masks_path)
        video_masks_path = os.path.join(self.dataset_path, '/'.join(video_masks_path.split('/')[2:]))
        ...
        depths = torch.stack(depths, dim=0)  # [T, 3, H, W]

        # in 'val', valid always satisfies
        return imgs, depths, target
...
def build(image_set, args):
    ...
    dataset = JHMDBSentencesDataset(img_folder, ann_file,
