...
transform_depth = T.Compose([
    T.Resize(360),
    T.ToTensor(),
    T.Normalize([0.5251, 0.2510, 0.2593], [0.3315, 0.2470, 0.1308],)
])
...
def main(args):
    ...
    data_folders = [os.path.join(root, split, "JPEGImages"),os.path.join(root, split, "JPEGDepth")]
            ...
            for obj_id in range(num_obj):
                ...
                for clip_id in range(0, video_len, crop_len):
                    ...
                    for t in clip_frames_ids:
                        frame = frames[t]
                        img_path = os.path.join(img_folder[0], video_name, frame + ".jpg")
                        depth_path = os.path.join(img_folder[1], video_name, frame + ".png")
                        img = Image.open(img_path).convert('RGB')
                        depth = Image.open(depth_path).convert('RGB')
                        origin_w, origin_h = img.size
                        imgs.append(transform(img))
                        depths.append(transform_depth(depth))

                    imgs = torch.stack(imgs, dim=0).to(args.device)
                    depths = torch.stack(depths, dim=0).to(args.device)
                    ...
                    with torch.no_grad():
                        with autocast(args.amp):
                            outputs = model([imgs], [depths], [exp], [target])
