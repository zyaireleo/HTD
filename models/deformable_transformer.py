...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, tgt, masks, pos_embeds, query_embed=None):
        ...
        if self.two_stage:
            ...
            tmp = rearrange(tgt, 'b t q c -> (b t q) c')
            ...
            #
            # q_pos = query_embed.reshape(-1, 256).contiguous()
            # q_pos = self.rgcn_conv(q_pos, edge_index, edge_type)
            # q_pos = self.agt_conv(q_pos, edge_index)
            # query_embed = q_pos.reshape(b * t, q, -1).contiguous()

            device = tmp.device
            edge_index, edge_type = initialize_edges_and_types(b, t, 5, device)
            tmp = self.rgcn_conv(tmp, edge_index, edge_type)
            # tmp = self.agt_conv(tmp, edge_index)
            tmp = rearrange(tmp, '(o q) c -> o q c', q=q)
...
if __name__ == '__main__':
    edge_index, edge_type = initialize_edges_and_types(2, 5, 5, 'cpu')
    rgcn_conv = RGCNConv(256, 256, 2)
    gat_conve = GATv2Conv(256, 256, 4, concat=False)
    out = rgcn_conv(torch.rand((50, 256)), edge_index, edge_type)
