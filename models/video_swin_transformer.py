...
def build_video_swin_backbone(args):
    ...
    # checkpoint = torch.load(args.visual_backbone_pretrained, map_location='cpu')
    # checkpoint_depth = torch.load(args.depth_backbone_pretrained, map_location='cpu')
    visual_backbone = Backbone(backbone, args.use_checkpoint, args.backbone_pretrained)
    # backbone_layers_state_dict = {
    #     (k.replace('backbone.0.', 'visual_backbone.0') if k.startswith('backbone.0.') else k): v for k, v in
    #     checkpoint['model'].items()}
    # backbone_layers_state_dict.update({
    #     k: v for k, v in checkpoint_depth['model'].items()
    #     if k not in backbone_layers_state_dict
    # })
    # backbone_layers_state_dict = {
    #     (k.replace('backbone.0.', 'depth_backbone.0') if k.startswith('backbone.0.') else k): v for k, v in
    #     checkpoint_depth['model'].items()}
    # missing_keys, unexpected_keys = visual_backbone.load_state_dict(backbone_layers_state_dict, strict=False)
    # missing_keys, unexpected_keys = visual_backbone.load_state_dict(backbone_layers_state_dict, strict=False)
    # visual_backbone.to(args.device)
    ...
    visual_backbone = Joiner(visual_backbone, position_embedding)
    depth_backbone = Backbone(args.depth_backbone, args.use_checkpoint, args.backbone_pretrained)
    depth_backbone = Joiner(depth_backbone, position_embedding)
    return visual_backbone, depth_backbone
