...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        tgt = tgt + 1e-5 * self.dropout(tgt2)
        tgt = self.norm(tgt)
...
class SgMg(nn.Module):
    ...
    def __init__(self, args, backbone, depth_backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_feature_levels - num_backbone_outs):
                input_proj_list.append(nn.Sequential(
                    ...
                    nn.GroupNorm(32, hidden_dim, eps=1e-5),
                ))
            self.input_proj = nn.ModuleList(input_proj_list)

            num_backbone_outs = len(depth_backbone.strides[-3:])
            for _ in range(num_backbone_outs):
                in_channels = depth_backbone.num_channels[-3:][_]
            ...
            for _ in range(num_feature_levels - num_backbone_outs):  # downsample 2x
                in_channels = depth_backbone.num_channels[-3:][-1]
                motion_proj_list.append(nn.Sequential(
    ...
    def build_controller(self):
        ...
        # self.mti = MTI(input_dim=self.hidden_dim, window_size=4, num_frame_queries=self.num_queries,
        #                num_queries=self.num_queries, nheads=8, dim_feedforward=2048,
        #                enc_layers=3, dec_layers=3)

        self.self_att = nn.ModuleList([AttentionLayer(self.hidden_dim, 8) for i in range(4)])
        self.vm_conv = nn.ModuleList(
            [nn.Conv2d(2 * channels, channels, kernel_size=1)
             for channels in self.backbone.num_channels[:2]]
        )
        self.f_norm = nn.ModuleList(
            [nn.LayerNorm(channels) for channels in self.backbone.num_channels[:2]]
        )

    def forward(self, samples: NestedTensor, depths, captions, targets):
        ...
        if not isinstance(depths, NestedTensor):
            depths = nested_tensor_from_videos_list(depths, 1 if self.training else 16)

        features, visual_pos = self.backbone(samples)
        motion_features, motion_position = self.depth_backbone(depths)
        ...
        if 'valid_indices' in targets[0]:
            ...
            for i, (vp, mp) in enumerate(zip(visual_pos, motion_position)):
                visual_pos[i] = vp.index_select(0, valid_indices)
        ...
        for l, (feat, pos_l, m_feat, m_pos) in enumerate(
                zip(features[-3:], visual_pos[-3:], motion_features[-3:], motion_position[-3:])):
            ...
            src_proj_l = rearrange(src_proj_l, '(b t) c h w ->b t c h w', t=t, h=h, w=w)
            # GNN Modality Fusion
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                m_pos = self.depth_backbone[1](NestedTensor(m_feat, None))
                n, c, h, w = src.shape
                ...
                src = rearrange(src, '(b t) c h w ->b t c h w', t=t, h=h, w=w)
        ...
        global_x = torch.stack(global_x, dim=0).to(text_embed.device)
        for layer in self.self_att:
            text_embed = layer(tgt=text_embed, memory=global_x)
        for i,layer in enumerate(self.self_att):
            text_embed = text_embed+F.dropout(layer(tgt=text_embed, memory=rearrange(global_x[i],'b c h w -> (h w) b c')))
        ...
        for i, (vf, df) in enumerate(zip(features[:2], motion_features[:2])):
            concatenated_tensors = torch.cat((vf.tensors, df.tensors), dim=1)

            b_, c_, h_, w_ = concatenated_tensors.shape

            conv_out = self.vm_conv[i](concatenated_tensors)
            conv_out = rearrange(conv_out, 'b c h w -> (b h w) c')
            conv_out = self.f_norm[i](conv_out)
            conv_out = rearrange(conv_out, '(b h w) c-> b c h w', b=b_, h=h_)
            coarse_feature.append(NestedTensor(conv_out, vf.mask))
        ...
        if self.training:
            ...
            if self.aux_loss:
                ...
                pred_masks = [self.mask_refine(x, coarse_feature) for x in pred_masks]
            ...
            pred_masks = self.mask_refine(pred_masks, coarse_feature)
            ...
            for idx_ in range(self.num_queries):
                pred_masks_ = self.mask_refine(pred_masks[:, idx_].flatten(0, 1), coarse_feature)
...
def build(args):
    ...
    # backbone
    if 'video_swin' in args.visual_backbone:
        from models.video_swin_transformer import build_video_swin_backbone
        visual_backbone, depth_backbone = build_video_swin_backbone(args)
    elif 'swin' in args.visual_backbone:
        from models.swin_transformer import build_swin_backbone
        visual_backbone, depth_backbone = build_swin_backbone(args)
    else:
        visual_backbone, depth_backbone = build_backbone(args)
